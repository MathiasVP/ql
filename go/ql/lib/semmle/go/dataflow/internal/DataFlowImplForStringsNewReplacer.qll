...
//     TaintedPathConfiguration() { this = "TaintedPathConfigurationABCABCABC" }
...
private int branch(NodeEx n1, Configuration conf) {
  result =
    strictcount(NodeEx n |
        flowOutOfCallNodeCand1(_, n1, _, n, conf) or
        flowIntoCallNodeCand1(_, n1, n, conf)
      ) + sum(getAdditionalFieldFlowBranchLimitTerm(_, _, n1, conf))
}

pragma[nomagic]
int getAdditionalFieldFlowBranchLimitTerm(
  DataFlowCall call, ParamNodeEx p1, ArgNodeEx n1, Configuration conf
) {
  flowIntoCallNodeCand1(call, n1, p1, conf) and
  result = getAdditionalFieldFlowBranchLimitTerm(call, p1.projectToNode(), n1.projectToNode())
}
...
private int join(NodeEx n2, Configuration conf) {
  result =
    strictcount(NodeEx n |
      ...
      ) + sum(getAdditionalFieldFlowBranchLimitTerm(_, n2, _, conf))
}
