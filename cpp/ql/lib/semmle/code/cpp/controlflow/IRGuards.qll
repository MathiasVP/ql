...
private class ScrutineeValueNumber extends ValueNumber {
  ...
  IRType getResultIRType() { result = scrutinee.getResultIRType() }

  predicate isGLValue() { scrutinee.isGLValue() }
}
...
private predicate unary_compares_eq(
  ...
  exists(AbstractValue dual, LogicalNotInstruction logicalNot, boolean inNonZeroCase0 |
    value = dual.getDualValue() and
    logicalNot = test.getAnInstruction() and
    unary_compares_eq(valueNumber(logicalNot.getUnary()), op, k, inNonZeroCase0, areEqual, dual)
  |
...
private predicate unary_simple_comparison_eq(
  ScrutineeValueNumber test, Operand op, int k, boolean inNonZeroCase, AbstractValue value
) {
  ...
  not test.getAnInstruction() = any(SwitchInstruction switch).getExpression() and
  (
...
class BuiltinExpectCallValueNumber extends ValueNumber {
...
private predicate builtin_expect_eq(
  ...
  exists(BuiltinExpectCallInstruction call, Instruction const, AbstractValue innerValue |
    ...
    compares_eq(valueNumber(call.getCondition()), left, right, k, areEqual, innerValue)
  |
    cmp instanceof CompareNEValueNumber and
    value = innerValue
    or
...
private predicate unary_builtin_expect_eq(
  ...
  exists(BuiltinExpectCallInstruction call, Instruction const, AbstractValue innerValue |
    int_value(const) = 0 and
    cmp.hasOperands(call.getAUse(), const.getAUse()) and
    unary_compares_eq(valueNumber(call.getCondition()), op, k, areEqual, inNonZeroCase, innerValue)
  |
    cmp instanceof CompareNEValueNumber and
    value = innerValue
    or
