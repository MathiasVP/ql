...
abstract class ScanfFunction extends Function {
   ...
   */
  predicate isSVariant() { this.getName().matches("%\\_s") }
}
...
class ScanfFunctionCall extends FunctionCall {
  ...
  Expr getOutputArgument(int n) {
    ...
    exists(ScanfFunction target | target = this.getScanfFunction() |
      ...
      then
        result =
          rank[n](Expr arg, int k |
            arg = this.getArgument(k) and
            not this.getArgument(k - 1).getUnspecifiedType() instanceof CharPointerType
          |
            arg order by k
          )
      else result = this.getArgument(this.getTarget().getNumberOfParameters() + n)
    )
  }
