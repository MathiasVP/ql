  ...
  (
    ...
    (
      ...
      exists(BuiltInInstruction builtIn |
        builtIn = instrTo and
        // __builtin_bit_cast
        builtIn.getBuiltInOperation() instanceof Cpp::BuiltInBitCast and
...
predicate simpleLocalFlowStepWithoutAliasing(Node nodeFrom, Node nodeTo, string model) {
  (
...
private predicate modelFlow(Node nodeFrom, Node nodeTo, string model) {
  exists(
    CallInstruction call, DF::DataFlowFunction func, Cpp::FunctionInput modelIn,
    Cpp::FunctionOutput modelOut
  |
...
private predicate reverseFlowOperand(Node nodeFrom, IndirectReturnOutNode nodeTo) {
  exists(Operand address, int indirectionIndex | nodeHasOperand(nodeTo, address, indirectionIndex) |
...
predicate indirectionOperandFlow(RawIndirectOperand nodeFrom, Node nodeTo) {
  nodeFrom != nodeTo and
  (
 ...
 */
pragma[nomagic]
predicate isStoredToBetween(Instruction instr, Operand operand) {
...
predicate indirectionInstructionFlow(RawIndirectInstruction nodeFrom, IndirectOperand nodeTo) {
  nodeFrom != nodeTo and
