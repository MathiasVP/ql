...
private import Stage3
private import Stage2
private import Stage0
private import Base

module FinalStage = Stage3;
...
cached
newtype TNode =
  TStageNode(FinalStage::Node node) { DataFlowImplCommon::forceCachingInSameStage() } or
 ...
 */
class StageNode extends Node, TStageNode {
  FinalStage::Node node;

  StageNode() { this = TStageNode(node) }
 ...
 */
private class StageArgumentNode extends ArgumentNode, StageNode {
  override FinalStage::ArgumentNode node;
...
class ParameterPosition = FinalStage::Position;

/** An argument position represented by an integer. */
class ArgumentPosition = FinalStage::Position;

class ReturnKind = FinalStage::ReturnKind;
 ...
 */
class IndirectReturnNode extends StageNode, ReturnNode {
  override FinalStage::ReturnNode node;
 ...
 */
private class SummaryOutNode extends OutNodeImpl, FlowSummaryNode {
...
predicate jumpStep(Node node1, Node node2) {
  exists(FinalStage::Node n1, FinalStage::Node n2 |
    node1 = TStageNode(n1) and
    node2 = TStageNode(n2) and
    FinalStage::jumpStep(n1, n2)
  )
  or
...
predicate storeStepImpl(Node node1, Content c, Node node2, boolean certain) {
  exists(FinalStage::Node n1, FinalStage::Node n2 |
    node1 = TStageNode(n1) and
    node2 = TStageNode(n2) and
    FinalStage::storeStep(n1, c, n2, certain)
  )
  or
  // models-as-data summarized flow
...
predicate nodeHasOperand(Node node, Operand operand, int indirectionIndex) {
  exists(FinalStage::Node n |
    ...
    FinalStage::nodeHasOperand(n, operand, indirectionIndex)
  )
}
...
predicate nodeHasInstruction(Node node, Instruction instr, int indirectionIndex) {
  exists(FinalStage::Node n |
    node = TStageNode(n) and
    FinalStage::nodeHasInstruction(n, instr, indirectionIndex)
  )
}
...
private newtype TDataFlowCallable =
  TStageCallable(FinalStage::DataFlowCallable callable) {
...
class DataFlowCallable extends TDataFlowCallable {
   ...
   */
  Cpp::Declaration asSourceCallable() { this = TStageCallable(result) }
 ...
 */
class SourceCallable extends DataFlowCallable, TStageCallable {
  Cpp::Declaration decl;

  SourceCallable() { this = TStageCallable(decl) }
...
class DirectParameterNode extends StageParameterNode {
  DirectParameterNode() {
    exists(ParameterPosition pos |
      this.isParameterOf(_, pos) and
      pos.getIndirectionIndex() = 0
    )
  }
}

class IndirectParameterNode extends StageParameterNode {
...
private newtype TDataFlowCall =
  TNormalCall(FinalStage::DataFlowCall call) or
  TSummaryCall(
...
private class NormalCall extends DataFlowCall, TNormalCall {
  private FinalStage::DataFlowCall call;
...
module IsUnreachableInCall {
  ...
  predicate isUnreachableInCall(NodeRegion block, DataFlowCall call) {
    ...
    |
      ...
      right = valueNumber(paramNode.asInstruction()).getAUse() and
      argval = arg.getValue()
    |
...
private IRVariable getIRVariableForParameterNode(ParameterNode p) {
  result.getAst() = p.(StageParameterNode).getParameter(_)
}
...
private predicate parameterNodeHasSourceVariable(ParameterNode p, Stage2::SourceVariable v) {
  ...
  exists(FinalStage::Position pos | p.isParameterOf(_, pos) |
