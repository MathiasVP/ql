...
predicate nodeIsHidden(Node n) {
  ...
  or
  // Any `IRFunction` generated by a `Function` which returns a value has
  // exactly one `ReturnValue` instruction. Each `return` statement then
  // generates a `Store` instruction that writes to the `IRFunction`'s
  // `IRReturnVariable`. These `Store` instructions are always present in the
  // path explanation since those dataflow nodes satisfy `asDefinition()` or
  // `asIndirectDefinition()` (which are explicitly marked as never to be
  // hidden in `neverSkipInPathGraph`). The location of the `ReturnValue`
  // instruction (and thus its operands) is the location of the function which
  // looks weird in path explanations. So we hide this node from path
  // explanations.
  n.(IndirectOperand)
