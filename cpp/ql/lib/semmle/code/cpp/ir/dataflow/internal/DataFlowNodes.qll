...
class FinalParameterNode extends Node, TFinalParameterNode {
  SsaImpl::FinalParameterUse use;

  FinalParameterNode() { this = TFinalParameterNode(use) }
  ...
  final int getArgumentIndex() { result = use.getArgumentIndex() }

  override Declaration getFunction() { result = use.getFunction() }

  override DataFlowCallable getEnclosingCallable() {
    result.asSourceCallable() = this.getFunction()
  }

  override Type getType() { result = use.getType() }
