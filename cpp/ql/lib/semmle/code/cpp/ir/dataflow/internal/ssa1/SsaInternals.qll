...
private import semmle.code.cpp.ir.dataflow.internal.ssa0.SsaInternals as SsaInternals0
private import semmle.code.cpp.ir.dataflow.internal.SsaInternalsCommon

private module SourceVariables {
  newtype TCount =
    TPreciseCount(int n) { n = [0 .. 1] } or
    TImpreciseCount()

  class Count extends TCount {
    string toString() { none() }

    predicate abstractionOf(int n) { none() }
  }

  class PreciseCount extends Count, TPreciseCount {
    int n;

    PreciseCount() { this = TPreciseCount(n) }

    override string toString() { result = n.toString() }

    override predicate abstractionOf(int n0) { n = n0 }
  }

  class ImpreciseCount extends Count, TImpreciseCount {
    override string toString() { result = "imprecise" }

    override predicate abstractionOf(int n) {
      not exists(TPreciseCount(n)) and
      (isDef(_, _, _, _, _, n) or isUse(_, _, _, _, n))
    }
  }
  ...
  private newtype TSourceVariable =
    TSourceIRVariable(BaseIRVariable baseVar, Count indirectionIndex) or
    TCallVariable(AllocationInstruction call, Count indirectionIndex)
  ...
  abstract class SourceVariable extends TSourceVariable {
    ...
    abstract Count getIndirection();
  }

  class SourceIRVariable extends SourceVariable, TSourceIRVariable {
    BaseIRVariable var;
    Count indirectionIndex;

    SourceIRVariable() { this = TSourceIRVariable(var, indirectionIndex) }
    ...
    override Count getIndirection() { result = indirectionIndex }
  }
  ...
  class CallVariable extends SourceVariable, TCallVariable {
    ...
    CallVariable() { this = TCallVariable(call, indirectionIndex) }
...
private newtype TDefOrUseImpl =
  TDefImpl(Operand address, Count indirectionIndex) {
    exists(SsaInternals0::Def def, int indirectionIndex0 |
      isDef(_, _, address, _, _, indirectionIndex0) and
      def.getAddressOperand() = address and
      indirectionIndex.abstractionOf(indirectionIndex0)
    )
  } or
  TUseImpl(Operand operand, Count indirectionIndex) {
    exists(int indirectionIndex0 |
      isUse(_, operand, _, _, indirectionIndex0) and
      indirectionIndex.abstractionOf(indirectionIndex0) and
...
class DefImpl extends DefOrUseImpl, TDefImpl {
  Operand address;
  Count indirectionCount;

  DefImpl() { this = TDefImpl(address, indirectionCount) }
...
class UseImpl extends DefOrUseImpl, TUseImpl {
  ...
  UseImpl() { this = TUseImpl(operand, indirectionCount) }
...
private module SsaInput implements SsaImplCommon::InputSig {
  ...
  predicate variableWrite(IRBlock bb, int i, SourceVariable v, boolean certain) {
    ...
    exists(DefImpl def | def.hasIndexInBlock(bb, i, v) and variableWriteCand(bb, i, v) |
