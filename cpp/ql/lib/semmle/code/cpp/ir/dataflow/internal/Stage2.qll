...
module Stage2 implements StageSig {
   ...
   */
  private predicate isModifiableByCall(ArgumentOperand operand, int indirectionIndex) {
  ...
  private predicate isWrite(Stage0::Node value, Operand address, boolean certain) {
  ...
  private predicate isAdditionalConversionFlow(Operand opFrom, Instruction instrTo) {
  ...
  private newtype TBaseSourceVariable =
  ...
  final private class BaseSourceVariable = AbstractBaseSourceVariable;

  private class BaseIRVariable extends AbstractBaseSourceVariable, TBaseIRVariable {
    IRVariable var;
  ...
  private class BaseCallVariable extends AbstractBaseSourceVariable, TBaseCallVariable {
  ...
  abstract private class BaseSourceVariableInstruction extends Instruction {
  ...
  additional class SourceVariable extends TSourceVariable {
  ...
  abstract additional class DefImpl extends TDefImpl {
  ...
  abstract additional class UseImpl extends TUseImpl {
    int indirectionIndex;
   ...
   * a function body.
   */
  private class InitialGlobalValue extends NodeImpl, TInitialGlobalValue {
    GlobalDef globalDef;
   ...
   */
  private class FinalGlobalValue extends NodeImpl, TFinalGlobalValue {
    GlobalUse globalUse;
   ...
   */
  private class GlobalUse extends UseImpl, TGlobalUse {
   ...
   */
  private class GlobalDefImpl extends DefImpl, TGlobalDefImpl {
    GlobalLikeVariable global;
    IRFunction f;
   ...
   */
  private predicate adjacentDefRead(IRBlock bb1, int i1, SourceVariable sv, IRBlock bb2, int i2) {
   ...
   */
  private predicate defToNode(
    Node node, Def def, SourceVariable sv, IRBlock bb, int i, boolean uncertain
  ) {
  ...
  private predicate modeledFlowBarrier(Node n) {
    ...
    |
      ...
      exists(callOutput(call, output, _)) and
  ...
  private predicate ssaFlow(Node nodeFrom, Node nodeTo) {
  ...
  private module SsaInput implements SsaImplCommon::InputSig<Location> {
    ...
    class SourceVariable = Stage2::SourceVariable;
  ...
  abstract additional class Def extends SsaDef, TDef {
    DefinitionExt def;
  ...
  private class GlobalDef extends Def {
  ...
  private newtype TNode =
    ...
    TPostUpdateFieldNode(Operand operand, int indirectionIndex) {
  ...
  abstract private class NodeImpl extends TNode {
    ...
    Variable asVariable(int indirectionIndex) { none() }
  }

  final class Node = NodeImpl;

  private class Node1 extends NodeImpl, TNode1 {
    Stage1::Node n;
  ...
  additional Node inject(Stage1::Node n) { result = TNode1(n) }
   ...
   */
  class VariableNode extends NodeImpl, TGlobalLikeVariableNode {
    Variable v;
    int indirectionIndex;
    ...
    override string toString() { result = this.stars() + v.toString() }

    final override predicate isGLValue() { none() }

    final override string stars() { result = repeatStars(indirectionIndex) }

    final override predicate hasIndexInBlock(IRBlock block, int index) { none() }

    final override Cpp::Variable asVariable(int indirectionIndex_) {
      indirectionIndex = indirectionIndex_ and
      result = v
    }
  }
   ...
   */
  private class FinalParameterNode extends NodeImpl, ReturnNodeImpl, TFinalParameterNode {
    Parameter p;
    int indirectionIndex;
  ...
  class FinalParameterNodeForTesting = FinalParameterNode;

  /**
   * INTERNAL: Do not use.
   */
  private class PostUpdateFieldNode extends PostUpdateNodeImpl, TPostUpdateFieldNode {
    int indirectionIndex;
    Operand operand;

    PostUpdateFieldNode() { this = TPostUpdateFieldNode(operand, indirectionIndex) }

    FieldAddress getFieldAddress() { operand = result.getObjectAddressOperand() }
    ...
    final override Location getLocation() { result = this.getPreUpdateNode().getLocation() }
   ...
   */
  additional class SsaPhiInputNode extends NodeImpl, TSsaPhiInputNode {
    PhiNode phi;
    IRBlock block;
  ...
  additional SsaPhiNode ssaPhiNode(PhiNode phi) { result = TSsaPhiNode(phi) }

  additional SsaPhiInputNode ssaPhiInputNode(PhiNode phi, IRBlock block) {
   ...
   */
  class SsaPhiNode extends NodeImpl, TSsaPhiNode {
    PhiNode phi;
  ...
  predicate nodeIsHidden(Node node) {
    exists(Stage1::Node n | node = TNode1(n) and Stage1::nodeIsHidden(n))
    or
    node instanceof FinalGlobalValue
    or
    node instanceof InitialGlobalValue
    or
    node instanceof SsaPhiInputNode
  }
  ...
  abstract private class OutNodeImpl extends NodeImpl {
    DataFlowCall call;
  ...
  private class Stage1OutNode extends OutNodeImpl, Node1 {
  ...
  abstract private class PostUpdateNodeImpl extends NodeImpl {
    abstract Node getPreUpdateNode();

    abstract int getIndirectionIndex();
  }

  final class PostUpdateNode = PostUpdateNodeImpl;

  additional class ArgumentOutNode extends PostUpdateNodeImpl, OutNodeImpl, TArgumentOutNode {
    ArgumentOperand operand;
    int argumentIndex;
    int indirectionIndex;

    ArgumentOutNode() {
      this = TArgumentOutNode(operand, indirectionIndex) and
      call.getArgumentOperand(argumentIndex) = operand
    }
  ...
  private class Stage1PostUpdateNode extends PostUpdateNodeImpl, Node1 {
    override Stage1::PostUpdateNode n;

    final override Node getPreUpdateNode() { result = TNode1(n.getPreUpdateNode()) }

    final override int getIndirectionIndex() { result = n.getIndirectionIndex() }
  ...
  abstract class ReturnKind extends TReturnKind {
    ...
    abstract predicate isReturnValue(int indirectionIndex);

    abstract predicate isArgument(int argumentIndex, int indirectionIndex);
  }
  ...
  class Stage1ReturnKind extends ReturnKind, TStage1ReturnKind {
    ...
    final override predicate isReturnValue(int indirectionIndex) {
      indirectionIndex = this.getIndirectionIndex()
    }

    final override predicate isArgument(int argumentIndex_, int indirectionIndex_) { none() }
  }

  private class ArgReturnKind extends ReturnKind, TArgReturnKind {
    int argumentIndex;
    int indirectionIndex;
    ...
    final override predicate isArgument(int argumentIndex_, int indirectionIndex_) {
      this.getArgumentIndex() = argumentIndex_ and
      this.getIndirectionIndex() = indirectionIndex_
    }

    final override predicate isReturnValue(int indirectionIndex_) { none() }
  }

  abstract private class ReturnNodeImpl extends NodeImpl {
  ...
  predicate storeStep(Node node1, Content c, Node node2, boolean certain) {
    exists(
      PostUpdateFieldNode postFieldUpdate, int indirectionIndex1, int numberOfLoads,
   ...
   * flow graph.
   */
  additional class UninitializedNode extends Node {
    Cpp::LocalVariable v;

    UninitializedNode() {
      exists(Def def, SourceVariable sv |
        def.getIndirectionIndex() = 0 and
        def.getValue().asInstruction() instanceof UninitializedInstruction and
        defToNode(this, def, sv, _, _, _) and
        v = sv.getBaseVariable().(BaseIRVariable).getIRVariable().getAst()
      )
    }
  ...
  additional predicate jumpStep(Node n1, Node n2) {
    exists(GlobalLikeVariable v |
      exists(GlobalUse globalUse |
        v = globalUse.getVariable() and
        n1.(FinalGlobalValue).getGlobalUse() = globalUse
      |
        v = n2.asVariable(globalUse.getIndirection())
      )
      or
      exists(GlobalDef globalDef |
        v = globalDef.getVariable() and
        n2.(InitialGlobalValue).getGlobalDef() = globalDef
      |
        v = n1.asVariable(globalDef.getIndirection())
      )
    )
  }
}
