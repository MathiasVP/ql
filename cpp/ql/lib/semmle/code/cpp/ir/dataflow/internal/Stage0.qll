...
module Stage0 implements StageSig {
  ...
  class VariableNode extends NodeImpl {
    VariableNode() { none() }
   ...
   */
  abstract private class InstructionNodeImpl extends NodeImpl {
    Instruction instr;
    ...
    override string toString() { result = instructionToString(instr) }
   ...
   */
  abstract private class OperandNodeImpl extends NodeImpl {
    Operand op;
    ...
    final override string toString() { result = operandToString(op) }
   ...
   */
  abstract private class InstructionDirectParameterNodeImpl extends ParameterNodeImpl instanceof InstructionNode
  {
    InitializeParameterInstruction instr;

    InstructionDirectParameterNodeImpl() { instr = super.getInstruction() }
  ...
  private class InstructionDirectParameterNode = InstructionDirectParameterNodeImpl;
  ...
  private class ExplicitParameterInstructionNode extends InstructionDirectParameterNodeImpl {
    ...
    final override string toString() { result = instr.getParameter().toString() }
  }
   ...
   */
  private class BodyLessParameterNodeImpl extends NodeImpl, TBodyLessParameterNodeImpl,
    ParameterNodeImpl
  {
    Parameter p;

    BodyLessParameterNodeImpl() { this = TBodyLessParameterNodeImpl(p) }

    override Declaration getEnclosingCallable() { result = this.getFunction() }

    override Declaration getFunction() { result = p.getFunction() }

    override DataFlowType getType() { result = p.getUnderlyingType() }
    ...
    final override string toString() { result = p.toString() }
    ...
    final override predicate isBodyless() { any() }
  }
