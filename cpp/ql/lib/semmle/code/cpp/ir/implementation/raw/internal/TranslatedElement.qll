...
abstract class TranslatedElement extends TTranslatedElement {
  ...
  /**
   * Holds if `tag` with completion `c` is a potential last element executed
   * within this element.
   */
  pragma[nomagic]
  abstract predicate last(InstructionTag tag, Completion c);
   ...
   */
  abstract Instruction getInstructionSuccessor(InstructionTag tag, EdgeKind kind, Completion c);
...
module Consistency {
  query predicate lastIsGeneratedByElement(TranslatedElement te, InstructionTag tag, string msg) {
    not te.hasInstruction(_, tag, _) and
    te.last(tag, _) and
    msg = tag.toString() + " is last, but is not generated by " + te.toString()
  }
}
